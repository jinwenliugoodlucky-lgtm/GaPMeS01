...
class GaussianAdapter(nn.Module):
    ...
    def __init__(self, cfg: GaussianAdapterCfg):
        ...
        self.sh_mask 是一个 shape 为 (self.d_sh,) 的张量，初始值全为 1。
        self.d_sh = (self.cfg.sh_degree + 1) ** 2，即球谐系数总数（比如 3阶就是16）。
        for 循环中，degree 从 1 到 sh_degree，将每一阶（非0阶）的球谐分量掩码设置为 0.1 * 0.25**degree，远小于1。
    ...
    def forward(
        ...
        if input_images is not None:
            ...
            sh[..., 0] = sh[..., 0] + RGB2SH(imgs)  #在这里将每个像素的RGB颜色加到对应的球谐系数的0阶分量上
